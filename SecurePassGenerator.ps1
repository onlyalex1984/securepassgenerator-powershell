...
# Define Swedish characters using Unicode escape sequences
...
function New-MemorablePassword {
    ...
    # Generate password with specified number of words
    ...
    # Add a number if enabled (always add if selected)
    if ($IncludeNumbers) {
        $result += $random.Next(0, 10).ToString()
    }
    
    # Add a special character if enabled (always add if selected)
    if ($IncludeSpecial) {
        $specialChars = "!@#-?_"
        $result += $specialChars[$random.Next(0, $specialChars.Length)]
    }
...
# Function to check if a password has been pwned using the HIBP API
...
# Function to show phonetic codes in a popup window
...
# Function to add phonetic buttons to the UI
...
# Function to push password to Password Pusher API
...
# Function to apply light theme styling
...
# Variables for countdown timers and state tracking
$script:hibpCountdown = 10
$script:pushCountdown = 10
$script:currentPasswordPushed = $false
$script:currentPasswordChecked = $false

# Create timers for rate limiting API calls with countdown
...
# Password Pusher button handler
...
# Have I Been Pwned button handler
...
# Create context menu for the dropdown button
